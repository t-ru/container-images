...
# This file is based on docker-entrypoint.sh from mariadb-10.3.18.tar.gz, which is GPLv2.
# Adjusted for SUSE mariadb package.
...
if [ "$1" = 'mysqld' -a -z "$wantHelp" ]; then
	...
	if [ ! -d "$DATADIR/mysql" ]; then
		...
		mysql_install_db --datadir="$DATADIR" --auth-root-authentication-method=normal --rpm
		echo -n `mysql --version | sed -e 's|.*Distrib \(.*\)-MariaDB.*|\1|g' -e 's|\.[0-9]\+$||'` > "$DATADIR"/mysql_upgrade_info
